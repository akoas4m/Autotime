...
class Dataset_OBS_FD01(Dataset):
    ...
    def __read_data__(self):
        ...
        try:
            ...
            self.data_stamp = torch.zeros((border2 - border1, 1))
...
class Dataset_Solar(Dataset):
    ...
    def __getitem__(self, index):
        ...
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
        sequence_x = self.data_x[s_begin:s_end, feat_id:feat_id + 1]
        sequence_y = self.data_y[r_begin:r_end, feat_id:feat_id + 1]
        sequence_x_mark = torch.zeros((sequence_x.shape[0], 1))
        sequence_y_mark = torch.zeros((sequence_x.shape[0], 1))

        return sequence_x, sequence_y, sequence_x_mark, sequence_y_mark
...
class Dataset_Preprocess(Dataset):
    ...
    def __read_data__(self):
        df_raw = pd.read_csv(os.path.join(self.root_path, self.data_path))
        df_stamp = df_raw[['dtime' if 'dtime' in df_raw.columns else 'date']]
        df_stamp['date'] = pd.to_datetime(df_stamp['dtime' if 'dtime' in df_raw.columns else 'date']).apply(str)
    ...
    def __getitem__(self, index):
        ...
        elif self.data_set_type in ['ETTm1', 'ETTm2', 'OBS_FD01_cleaned']:
